...
return Application::configure(basePath:dirname(__DIR__))->withRouting(web:__DIR__.'/../routes/web.php',commands:__DIR__.'/../routes/console.php',health:'/up')->withProviders([App\Providers\AppServiceProvider::class])->withMiddleware(function(Middleware $m){$m->alias(['role'=>App\Http\Middleware\RoleMiddleware::class]);})->withExceptions(function(Exceptions $e){})->create();
